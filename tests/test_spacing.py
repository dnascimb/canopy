...
from canopy.models import Group, Plant, PlantStatus, Space, SpaceStage, Strain
...
def test_occupancy_counts_plants(app):
    """A space holds a number of plants. There is no floor area in the model."""
...
def test_space_form_sets_stage_and_what_it_doubles_up_for(client):
    ...
    assert s.stage.value == "vegetative" and s.capacity == 9
    assert s.hosts == frozenset({SpaceStage.vegetative, SpaceStage.clone, SpaceStage.flowering})
...
def test_api_spaces_and_locations(client):
    ...
    assert "area_sqft" not in flower and "fits" not in flower
    assert flower["count"] == 10
    load = client.get(f"/api/v1/spaces/{flower['id']}/load").get_json()
    assert load[-1]["count"] == 0
...
def test_backup_roundtrip_keeps_new_fields(client):
    ...
    assert db.session.query(Space).filter_by(name="Flower Room").one().stage.value == "flowering"
    assert (
